...
- [ ] **Manual (dashboard, one-time):** Google OAuth. `gcloud` cannot do this: the IAP OAuth Admin API (the only CLI path to OAuth clients) requires a Cloud Organization — this account has none — and Google is shutting that API down (discontinued for new projects Jan 2026; permanently shut down Mar 2026). Create a **Google Cloud Console → APIs & Services → OAuth consent screen + Credentials → OAuth client ID (Web)** with authorized redirect URI `https://hqgamvpcowjjgxkchtzu.supabase.co/auth/v1/callback`, then paste the client id/secret into **Supabase → Authentication → Providers → Google**. Email magic link works without this.
