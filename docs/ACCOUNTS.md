...
- **Replay (client-only):** stepping through move history is a pure view concern. `src/engine/replay.ts` (`boardAtMove(moves, index)`) reconstructs any historical position by replaying the stored `{ chipId, to }` list from the standard starting position — Outwit has no captures and a deterministic setup, so no server round-trip or stored snapshots are needed. `useReplay` (in `GamePage.tsx`) drives the mode; `ReplayControls` provides buttons, a slider, and keyboard (←/→/Home/End/Escape); clicking a move in `MoveHistoryPanel` jumps there. While reviewing, the board shows the reconstructed state and all input is ignored. **The real game, clocks, server state, and match record are never touched** — the store's `state`/`moveHistory` are asserted unchanged in `GamePage.test.tsx`.
